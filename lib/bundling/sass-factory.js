...
function sassFactory(entrypoint, options) {
  if (!options) {
    options = {};
  }
    ...
    sass.render({
      ...
      if (err) {
        ...
        if (environment === 'development') {
          callback(null, formatTextAsCSS(message));
        } else {
          callback(err);
        }
      } else {
...
module.exports.formatTextAsCSS = formatTextAsCSS;
