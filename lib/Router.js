...
Router.prototype = _.extend(Router.prototype, {
  ...
  // Start the store.
  //
  // If `singlePage` is `false`, the navigation store will not use
  // `pushState`-type page management; defaults to `true`.
  //
  // If `referrer` is provided, becomes the referrer on the first page view
  // event.
  //
  start: function(options) {
    ...
    if (options.singlePage === false) {
      this._strategy = new History.LocationStrategy();
    } else {
  ...
  _handleHistoryChange: function(url, state, type) {
    ...
    if (result && result.length > 0) {
      ...
      if (!previousUrl && this._referrer) {
        previousUrl = this._referrer;
        this._referrer = null;
      }
